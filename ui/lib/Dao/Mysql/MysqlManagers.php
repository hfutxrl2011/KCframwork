...
class MysqlManagers
{
	...
	public static function getTbNo($identifier)
	{
		...
		$tag = (Sign::mod($sign, (EnvConf::$dbConf['db_num'] * EnvConf::$dbConf['tb_num']))) % EnvConf::$dbConf['tb_num'];
		return $tag;
	}
	...
	public static function getDbNo($identifier)
	{
		$sign = $identifier;
		is_numeric($identifier) || $sign = Sign::sign64($identifier);
		$tag = intval((Sign::mod($sign, (EnvConf::$dbConf['db_num'] * EnvConf::$dbConf['tb_num']))) / EnvConf::$dbConf['tb_num']);	
		return $tag;
	}
	 ...
	 */
	public function fetchMysqlHandler($identifier, $tag = NULL ,$dbtag = NULL) 
	{
		if($this->_isConnected) 
		{
			$this->_mysqli->close();
		...
		if(!is_null($dbtag))
		{
			if(!isset(EnvConf::$dbConf['server'][$dbtag][intval($tag)]))
			{
				KC_LOG_WARNING(__FUNCTION__ . " failed, invalid tag received [ tag: $tag ,dbtag: $dbtag ]");
				return false;
			}
			$arrMysqlServer = EnvConf::$dbConf['server'][$dbtag][intval($tag)];
		}
		else if(!is_null($tag)) 
		{
			if(!isset(EnvConf::$dbConf['server'][Env::getIDC()][intval($tag)]))
			{
				KC_LOG_WARNING(__FUNCTION__ . " failed, invalid tag received [ tag: $tag ]");
				return false;
			}
			$arrMysqlServer = EnvConf::$dbConf['server'][Env::getIDC()][intval($tag)];
		}
		else 
		{
		...
		for($i = 0; $i < $totalNum; $i++) 
		{
			...
			$this->_mysqli->set_charset(EnvConf::$dbConf['charset']);
